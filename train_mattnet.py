...
def lossFun(loader, optimizer, model, mm_crit, att_crit, opt, iter):
  ...
  # return 
  return loss.data.item(), T, data['bounds']['wrapped']
...
def main(args):
  ...
  head_feats_dir=osp.join(MATTNET_DIR, 'cache/feats/', opt['dataset_splitBy'], 'rcnn', feats_dir)
  loader.prepare_rcnn(head_feats_dir, args)
  ann_feats = osp.join(MATTNET_DIR, 'cache/feats', opt['dataset_splitBy'], 'rcnn', 
