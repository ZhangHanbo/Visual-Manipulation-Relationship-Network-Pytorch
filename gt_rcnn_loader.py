...
sys.path.insert(0, osp.join(MATTNET_DIR, 'lib'))
...
class GtRCNNLoader(Loader):
  ...
  def prepare_rcnn(self, head_feats_dir, args):
    """
    Arguments:
  ...
  def getBatch(self, split, opt):
    ...
    max_len = max((pos_labels != 0).sum(1).max().data.item(), 
                  (neg_labels != 0).sum(1).max().data.item())
  ...
  def fetch_grid_feats(self, boxes, net_conv, im_info):
    ...
    - fc7   (n, 2048, 7, 7) 
    """
    img_scale = im_info[0][2]
    obj_boxes = torch.from_numpy(boxes).type(torch.FloatTensor).cuda()
    obj_boxes = obj_boxes.unsqueeze(0)
    pool5, fc7 = self.RCNN.box_to_spatial_fc7(net_conv, obj_boxes, img_scale)
    return pool5, fc7
