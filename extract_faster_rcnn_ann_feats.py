...
def ann_to_pool5_fc7(RCNN, ann, base_feat, im_info):
    ...
    obj_boxes = torch.from_numpy(box).cuda()
    obj_boxes = obj_boxes.unsqueeze(0)
    pool5, fc7 = RCNN.box_to_pool5_fc7(torch.from_numpy(base_feat).cuda(), box, im_info[0][2])  # (1, 2048)
    return pool5, fc7
...
def main(args, rcnn_args):
    ...
    feats_h5 = osp.join('cache/feats', dataset_splitBy, 'rcnn', file_name)
    ...
    base_feats_dir = osp.join('cache/feats/', dataset_splitBy, 'rcnn', feats_dir)
