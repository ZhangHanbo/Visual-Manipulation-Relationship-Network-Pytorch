...
def ann_to_pool5_fc7(rcnn, ann, net_conv, im_info):
  """
  Arguments:
    ann: object instance
    net_conv: float32 (1, 1024, H, W)
  ...
  box = np.array([ann['box']])  # [[xywh]]
  box = xywh_to_xyxy(box)  # [[x1y1x2y2]]
  pool5, fc7 = rcnn.box_to_pool5_fc7(Variable(torch.from_numpy(net_conv).cuda()), im_info, box)  # (1, 2048)
  return pool5, fc7
...
def main(args):
  ...
  for i, image in enumerate(images):
    ...
    for ann_id in ann_ids:
      ...
      ann_pool5, ann_fc7 = ann_to_pool5_fc7(RCNN, ann, net_conv, im_info)
      ann_h5_id = ann['h5_id']
