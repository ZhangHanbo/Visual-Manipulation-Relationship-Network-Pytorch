    ...
    def init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                weights_xavier_init(m, distribution='uniform')
...
class BiFPNModule(nn.Module):
    ...
    def forward(self, inputs):
        ...
        for i in range(levels - 1, 0, -1):
            pathtd[i - 1] = (w1[0, i-1]*pathtd[i - 1] + w1[1, i-1]*F.upsample(
                pathtd[i], size=(pathtd[i-1].size(-2), pathtd[i-1].size(-1)), mode='bilinear'))/(w1[0, i-1] + w1[1, i-1] + self.eps)
